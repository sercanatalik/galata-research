...
def trades(*args, **kwargs) -> pl.DataFrame:
    return market.trades(*args, **kwargs).collect()


def quotes(*args, **kwargs) -> pl.DataFrame:
...
def an_id_is_the_venues(tape):
    ...
    tape.write()
    assert trades("BTC", *DAY)["venue"].to_list() == ["hyperliquid", "rh_crypto"]
...
def the_trades_in_one_millisecond_keep_arrival_order(tape):
    ...
    tape.write()
    assert trades("BTC", *DAY)["trade_id"].to_list() == ["c", "b", "a"]
...
def the_trades_in_one_message_keep_the_messages_order(tape):
    ...
    tape.write()
    assert trades("BTC", *DAY)["trade_id"].to_list() == ["z", "a", "m"]
...
def a_tick_after_as_of_is_not_known(tape):
    ...
    tape.write()
    got = trades("BTC", *DAY, as_of=utc("2026-09-25T12:00"))
...
def every_quote_stands_even_when_it_repeats(tape):
    ...
    tape.write()
    assert quotes("BTC", *DAY).height == 2
...
def the_tick_engines_agree_row_for_row(tape):
    ...
    tape.write()
    assert_frame_equal(quotes("BTC", *DAY), market.quotes("BTC", *DAY, engine="duckdb").pl())
    assert_frame_equal(trades("BTC", *DAY), market.trades("BTC", *DAY, engine="duckdb").pl())
...
def a_known_ticker_outside_coverage_is_empty_with_the_columns(tape):
    ...
    got = trades("BTC", utc("2025-01-01T00:00"), utc("2025-01-02T00:00"))
    assert got.height == 0
