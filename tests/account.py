...
def each_dex_is_its_own_snapshot(tape):
    ledger(tape.root, [{"payload": snapshot(dex="")}, {"payload": snapshot(dex="xyz")}])
    assert margin()["dex"].to_list() == ["", "xyz"]
...
def a_unified_account_does_not_hold_equity(tape):
    ...
    assert margin()["equity_held"].to_list() == [False, True]
...
def the_mode_rows_are_skipped(tape):
    ...
    assert margin().height == 1


def another_schema_version_is_refused(tape):
